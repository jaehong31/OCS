...
if 'rot-mnist' in DATASET:
    ...
    config = {'exp_name': '-rcp',
              # ---COMMON----
              'num_tasks': 20, 'trial': TRIAL_ID, 'exp_dir': EXP_DIR,\
              'memory_size': 100, 'dataset': DATASET, 'device': DEVICE, 'momentum': 0.8,\
              'mlp_hiddens': HIDDENS, 'dropout': 0.1, 'lr_decay': 0.9,\
              'n_classes': 5,\

               # ----Seq Model-----
               'seq_lr': 0.15*0.85, 'stream_size': 20, 'seq_epochs': 1,\

               # ------OCS-------
               'ocspick': True, 'batch_size': 10, 'is_r2c': True, 'r2c_iter': 100,\
               'tau': 1000.0, 'ref_hyp': 0.5, 'select_type': 'ocs_select', 'coreset_base': False,
               }

    if 'imb-cifar' in DATASET:
        config['lr_decay'] = 0.875
        config['tau'] = 10.0
        config['ref_hyp'] = 0.1
